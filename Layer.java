...
public class Layer {
	...
	private ArrayList<Item> items = new ArrayList<Item>(); // all the items in
															// this layer
	...
	Item checkItemToPickup(int x, int y) {
		int[] playerPos = { x, y };
		for (Item i : items) {
			if (i.collidesWith(playerPos) && ((i instanceof Chest) || (i instanceof Key) || (i instanceof Coin) || (i instanceof Onion))) {
				return i;
			}
		}
		return null;
	 ...
	 * First: Choose a random x, y between the bounds of the screen, Second:
	 * make sure it doesn't overlap the other stairs or the items --( if it
	 * does, pick a different random x, y) Third: return the random x,y
	 */
